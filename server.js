...
const {
    ...
    addDepartment,
    updateEmployee
} = require('./untils/questions.js');
// dotenv.config();

function start() {
    inquirer.prompt(Start_Q).then((answer) => {
        switch (answer.choice) {
            ...
            case 'View all departments': viewDepartments();
                break;
            case 'Add new employee': addEmployeeFunc();
                break;
            case 'Add new role': addRoleFunc();
                break;
            case 'Add new department': addDepartmentFunc();
                break;
            case 'Update employee role': updateEmployeeFunc();
                break;
...
function addRoleFunc() { // console.log('add new role')
    let request = "SELECT * FROM department";
    // console.log("request")
    db.query(request, function (err, res) { // console.log("this is query")
        if (err) {
            throw err;
        }
        // console.log(res)
...
function addDepartmentFunc() {
    ...
    .then(function (response) {
        db.query('INSERT INTO department(department_name) VALUES (?)', response.department_name, function (err, res) {
            if (err) {
                throw err;
            }
            inquirer.prompt([{
...
function updateEmployeeFunc() {
    ...
    db.query(request, function (err, res) {
        ...
        let employeeList = res.map((x) => ({name: `${
                x.first_name
            } ${
                x.last_name
            }`, value: x.id}));
        ...
        db.query('SELECT * FROM roles', (err, res) => {
            ...
            let roleLists = res.map((x) => ({name: x.title, value: x.id}));
            const updateEmployeeQuestions = updateEmployee(employeeList, roleLists);
            inquirer.prompt(updateEmployeeQuestions).then(function (response) {
